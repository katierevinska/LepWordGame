...
void Model::SetModelByLevel(int level){
    if(level==1){
        std::vector<QPolygonF> way={{{{0,150},{450, 150},{450, 200},{0, 200}}}};
        hero.SetHero(1, {way[0].first().rx()+5, way[0].first().ry()-4});
        qDebug()<<hero.position;
        std::vector<Enemy> enemies={{1,{{75,146},{120, 146}}, Enemy::Bee},{1,{{200,146},{240, 146}}, Enemy::Bee}};
